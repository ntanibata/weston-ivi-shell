...
static void
test_layer_set_visibility_after_destroy_layer(struct test_context *ctx)
{
	const struct ivi_controller_interface *ctl = ctx->controller_interface;
	struct ivi_layout_layer *ivilayer;
	...
	ctl->layer_remove(ivilayer);
	iassert(ctl->layer_set_visibility(ivilayer, true) == IVI_FAILED);
}

static void
test_layer_set_opacity_after_destroy_layer(struct test_context *ctx)
{
	const struct ivi_controller_interface *ctl = ctx->controller_interface;
	struct ivi_layout_layer *ivilayer;
	...
	ctl->layer_remove(ivilayer);
	iassert(ctl->layer_set_opacity(
		    ivilayer, wl_fixed_from_double(0.5)) == IVI_FAILED);
}

static void
test_layer_set_orientation_after_destroy_layer(struct test_context *ctx)
{
	const struct ivi_controller_interface *ctl = ctx->controller_interface;
	struct ivi_layout_layer *ivilayer;
	...
	ctl->layer_remove(ivilayer);
	iassert(ctl->layer_set_orientation(
		    ivilayer, WL_OUTPUT_TRANSFORM_90) == IVI_FAILED);
}

static void
test_layer_set_dimension_after_destroy_layer(struct test_context *ctx)
{
	const struct ivi_controller_interface *ctl = ctx->controller_interface;
	struct ivi_layout_layer *ivilayer;
	...
	ctl->layer_remove(ivilayer);
	iassert(ctl->layer_set_dimension(ivilayer, 200, 300) == IVI_FAILED);
}

static void
test_layer_set_position_after_destroy_layer(struct test_context *ctx)
{
	const struct ivi_controller_interface *ctl = ctx->controller_interface;
	struct ivi_layout_layer *ivilayer;
	...
	ctl->layer_remove(ivilayer);
	iassert(ctl->layer_set_position(ivilayer, 20, 30) == IVI_FAILED);
}

static void
test_layer_set_source_rectangle_after_destroy_layer(struct test_context *ctx)
{
	const struct ivi_controller_interface *ctl = ctx->controller_interface;
	struct ivi_layout_layer *ivilayer;
	...
	ctl->layer_remove(ivilayer);
	iassert(ctl->layer_set_source_rectangle(
		    ivilayer, 20, 30, 200, 300) == IVI_FAILED);
}

static void
test_layer_set_destination_rectangle_after_destroy_layer(struct test_context *ctx)
{
	const struct ivi_controller_interface *ctl = ctx->controller_interface;
	struct ivi_layout_layer *ivilayer;
...
static void
test_layer_remove_duplicate(struct test_context *ctx)
{
	const struct ivi_controller_interface *ctl = ctx->controller_interface;
	struct ivi_layout_layer *ivilayer;
...
test_get_layer_after_destory_layer(struct test_context *ctx)
{
	...
	duplicatelayer = ctl->get_layer_from_id(IVI_TEST_LAYER_ID(0));
...
run_internal_tests(void *data)
{
	...
	test_layer_set_visibility_after_destroy_layer(ctx);
	test_layer_set_opacity_after_destroy_layer(ctx);
	test_layer_set_orientation_after_destroy_layer(ctx);
	test_layer_set_dimension_after_destroy_layer(ctx);
	test_layer_set_position_after_destroy_layer(ctx);
	test_layer_set_source_rectangle_after_destroy_layer(ctx);
	test_layer_set_destination_rectangle_after_destroy_layer(ctx);
