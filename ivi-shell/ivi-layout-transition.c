...
enum layout_transition_type{
    LAYOUT_TRANSITION_VIEW_MOVE_RESIZE,
    LAYOUT_TRANSITION_VIEW_FADE,
    LAYOUT_TRANSITION_LAYER_FADE,
    LAYOUT_TRANSITION_LAYER_MOVE,
    LAYOUT_TRANSITION_LAYER_VIEW_ORDER,
    LAYOUT_TRANSITION_MAX,
};

struct ivi_layout_transition {
    enum layout_transition_type type;
    void *private_data;
    void *user_data;
...
get_transition_from_type_and_id(enum layout_transition_type type, void* id_data)
{
...
create_layout_transition(void)
{
    ...
    transition->type = LAYOUT_TRANSITION_MAX;
    ...
    uint32_t duration)
{
    ...
    transition->type = LAYOUT_TRANSITION_VIEW_MOVE_RESIZE;
...
{
    int32_t start_pos[2] = {};
    ivi_layout_surfaceGetPosition(surface, start_pos);

    uint32_t start_size[2] = {};
    ivi_layout_surfaceGetDimension(surface, start_size);

    struct ivi_layout_transition* transition = NULL;

    transition = get_transition_from_type_and_id(LAYOUT_TRANSITION_VIEW_MOVE_RESIZE, surface);
    if(transition){
    ...
    uint32_t duration)
{
    ...
    transition->type = LAYOUT_TRANSITION_VIEW_FADE;
                                       ...
                                       uint32_t duration)
{
    ...
    transition = get_transition_from_type_and_id(LAYOUT_TRANSITION_VIEW_FADE, surface);
    if(transition){
        transition->time_start = 0;
        transition->time_duration = duration;
    ...
    uint32_t duration)
{
    ...
    transition->type = LAYOUT_TRANSITION_LAYER_MOVE;
...
{
    struct ivi_layout_transition* transition =
        get_transition_from_type_and_id(LAYOUT_TRANSITION_LAYER_MOVE, layer);
    if(transition){
        layout_transition_destroy(transition);
    }
}
                                    ...
                                    uint32_t duration)
{
    ...
    transition = get_transition_from_type_and_id(LAYOUT_TRANSITION_LAYER_FADE, layer);
    ...
    transition->type = LAYOUT_TRANSITION_LAYER_FADE;
                                            ...
                                            uint32_t duration)
{
    ...
    transition = get_transition_from_type_and_id(LAYOUT_TRANSITION_LAYER_VIEW_ORDER, layer);
    if(transition){
        /* update transition */
    ...
    transition->type = LAYOUT_TRANSITION_LAYER_VIEW_ORDER;
